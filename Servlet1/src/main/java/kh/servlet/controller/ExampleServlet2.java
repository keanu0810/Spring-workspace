...
/* 자바 파일이 여러파일, form 연결이 다수일 경우 / 위피로 연결*/
public class ExampleServlet2 extends HttpServlet {

	//Get 방식 요청을 (method"get"인 form 태그 제출) 처리하는 매서드
	@Override // Httpservlet 에서 가져와서 개발자에 맞게 다시 작성해주는 것이기 떄문에 재사용 Override 작성
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		/*
		 매개변수 req, resp
		 
		 HttpServletRequest
		  - 클라이언트 요청 시 자동을 ㅗ생성되는 객체
		 ...
		 HttpServletResponse
		  ...
		  --index.html 만들어서 제출된 이름과 아니 얻어오기
		  --> 제출된 이름, 나이 = 전달 매개변수(Parameter) == 전달된 값 보여줄 것
		  
		  --> 요청에 담긴 특정 파라미터 가져오기 name
		  ---> String HttpservleRequest.getParameter("name속성값");
		  
		 * */
		//<input type="text" name="inputName"> 에 작성되어 있는 제출된 값 얻어오기
		...
		/*
		 ...
		 -> index.html ExampleServlet2 서로 파일 취치 찾지 못해서 발생하는 에러
		...
		sb.append("<body>");
		sb.append(String.format("<h1> %s님의 나이는 %ㄴ 입니다.</h1>", inputName, inputAge));
		sb.append("</body>");
		sb.append("</html>");
